...
- (IBAction)clickToLogin:(NSButton *)sender
{
    ...
    //程序员调试专用===激活PDCA和SFC，可以选择
    if ([_userName.stringValue isEqualToString:@"admin"] && [_passWord.stringValue isEqualToString:@"admin"])
    {
         [self.window orderOut:self];
        
        [[NSNotificationCenter defaultCenter] postNotificationName:@"Show_PDCA_and_SFC_Notification" object:nil];
    ...
    //激活上传数据，检测上工序
        ...
        [self.window orderOut:self];
        //发送通知,激活按钮状态
        [[NSNotificationCenter defaultCenter] postNotificationName:@"Activate_SFC_Notification" object:nil];
    }
     //激活上传数据，不检测上工序
    else if ([_userName.stringValue isEqualToString:@"op1"]&&[_passWord.stringValue isEqualToString:@"op1"])
    {
        [self.window orderOut:self];
        //发送通知，激活SFC和PDCA
        [[NSNotificationCenter defaultCenter] postNotificationName:@"Activate_PDCA_and_SFC_Notification" object:nil];
